...
try
{
    ...
    string FolderOrigen = Dts.Variables["User::FolderOrigenVendedor"].Value.ToString();
    string tabla = Dts.Variables["User::TablaDestinoVendedor"].Value.ToString();
    ...
    foreach (string fileName in fileEntries)
    {
        ...
        while ((line = SourceFile.ReadLine()) != null)
        {
            if (counter > 0)
            {
                ...
                string query = "INSERT INTO " + tabla + " (codigoVendedor, nombreVendedor, sucursal) Values('" + campos[0] + "','" + campos[1] + "','" + campos[2] + "')";
